...
if (isset($_REQUEST['val1']) && isset($_REQUEST['val2']) && isset($_REQUEST['region']) && isset($_REQUEST['crop'])){
		...
		$m = mysql_query("select user_info.name,user_info.address,user_info.lat,user_info.lng,user_products.quantity,user_products.price,user_products.userid,user_products.productid,((user_info.lat-".$lat.")*(user_info.lat-".$lat.")+(user_info.lng-".$lng.")*(user_info.lng-".$lng.")) as dist from user_products INNER JOIN user_info ON user_products.userid=user_info.userid ORDER BY dist ASC");
		echo '<table style="border: 2px solid black;">';
		echo '<tr style="border:2px solid black;height:50px;text-align:center;">';
		echo '<td style="border:2px solid black;height:50px;text-align:center;">Name</td>';
		echo '<td style="border:2px solid black;height:50px;text-align:center;">Location</td>';
		echo '<td style="border:2px solid black;height:50px;text-align:center;">Contact Number</td>';
		echo '<td style="border:2px solid black;height:50px;text-align:center;">productid</td>';
		echo '<td style="border:2px solid black;height:50px;text-align:center;">quantity</td>';
		echo '<td style="border:2px solid black;height:50px;text-align:center;">price</td>';
		echo '</tr>';
		while($row=mysql_fetch_array($m)){
			if (($row['price'] >= $a) && ($row['price'] <= $b) && ($row['productid']==$d)){
				//$ro = mysql_fetch_array($m);
				echo '<tr>';
				echo "<td style='border:2px solid black;width:200px;height:50px;text-align:center;'>".$row['name']."</td>";
				echo "<td style='border:2px solid black;width:200px;height:50px;text-align:center;'>GachiBowli Hyderabad</td>";
				echo "<td style='border:2px solid black;width:200px;height:50px;text-align:center;'>9839567483</td>";
				echo "<td style='border:2px solid black;width:200px;height:50px;text-align:center;'>".$row['productid']."</td>";
				echo "<td style='border:2px solid black;width:200px;height:50px;text-align:center;'>".$row['quantity']."</td>";
				echo "<td style='border:2px solid black;width:200px;height:50px;text-align:center;'>".$row['price']."</td>";
				echo '</tr>';
			}
		}
		echo '</table>';
